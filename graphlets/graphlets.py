...
class Graphlets():
    ...
    def compute(self, r_cut, reduce=True, n_jobs=-1):
        if n_jobs == 1:
            G_total = np.asarray([
                self.decompose(compute_graph(x, r_cut, dims=self.dims, metric=self.metric), t)
                for t, x in enumerate(self.X)
            ])
        else:
            G_total = np.asarray(
                Parallel(n_jobs=n_jobs)(
                    delayed(self.decompose)(compute_graph(x, r_cut, dims=self.dims, metric=self.metric), t)
                    for t, x in enumerate(self.X)))
    ...
    def scan_r_cut(self, r_min, r_max, num=25, n_jobs=-1):
        ...
        if n_jobs == 1:
            for i, r in enumerate(np.linspace(r_min, r_max, num=num)):
                r_scan[i] = np.asarray([
                    nx.number_of_edges(compute_graph(x, r, dims=self.dims, metric=self.metric)) for x in self.X
                ]).mean()
        else:
            for i, r in enumerate(np.linspace(r_min, r_max, num=num)):
                r_scan[i] = np.asarray(
                    Parallel(n_jobs=n_jobs)(
                        delayed(nx.number_of_edges)(compute_graph(x, r, dims=self.dims, metric=self.metric))
